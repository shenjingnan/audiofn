...
- **语音合成（TTS）** — Qwen3-TTS-0.6B / 1.7B 零样本音色克隆：录一段 3–10 秒（建议 5–10 秒）参考音频 + 对应文本，即可用该音色合成任意文本（支持中英日韩等 10 语种）
...
音色克隆三步：录一段 3–10 秒（建议 5–10 秒）干净的参考音频 → 写下音频里说的原话（逐字）→ `--reference-wav` + `--reference-text` 一起传给 `tts run`。音色也可以在桌面端「音色库」录制保存，之后用 `--voice <音色id>` 复用。
