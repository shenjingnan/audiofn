...
#
# 编译参数与 release.yml 一致：只编译 qwen3_tts + qwen3_asr 两族模型
# + DEPLOYMENT_BUILD（spec 内嵌）+ NATIVE_CPU=OFF（可移植）。
# 产物体积与编译耗时随白名单变化（macOS 实测为分钟级）。
# 注意：上游 tag 命名 0.6.x 为 release-X.Y.Z（无 v 前缀），0.7.x 起为 vX.Y.Z。
#
# 平台：仅 macOS / Linux（Windows 已不在支持范围，无 .exe / CUDA DLL 处理）。
set -euo pipefail
...
triple() {
  ...
  case "$(uname -s)/$(uname -m)" in
    ...
    *) echo "不支持的平台: $(uname -s)/$(uname -m)（仅支持 macOS / Linux）" >&2; exit 1 ;;
  esac
  echo "$t"
}
...
if [ "${1:-}" = "--build" ]; then
  ...
  # CUDA 后端不启用（历史上只有 Windows 腿用 CUDA；macOS 走 Metal、Linux 走 CPU）
  ...
  cmake -S .audiocpp-src -B .audiocpp-build \
    -DAUDIOCPP_MODEL_SET=custom -DAUDIOCPP_MODELS=qwen3_tts,qwen3_asr \
  ...
  # 单配置生成器（Ninja/Make）产物落 bin/，多配置生成器落 bin/<Config>/，find 兜底两种布局
